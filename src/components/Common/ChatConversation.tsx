...
export const ChatConversation = ({
  ...
  const chatData = chats.find((chat) => chat.chatId === chatToken);

  useEffect(() => {
    if (messagesEndRef.current) {
      setTimeout(() => {
        messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
      }, 100); // A slight delay to ensure the scroll happens after render
    }
  }, [chatData, chatData?.messages]);

  if (!chatData) {
    return <div className="p-4 text-red-500">Chat no encontrado</div>;
  }
    ...
    <>
      ...
      >
        ...
        <div
          ...
          } border-b flex items-center gap-3 p-2 md:p-4`}
        >
          <button
            className={`${
              ...
              <div
                className={`${
                  themeMode === "light"
                    ? "hover:text-white"
                    : "hover:text-black"
                } px-5 py-2 rounded-xl flex items-center font-bold cursor-pointer bg-[rgba(13,188,115,0.1)] hover:bg-[#0DBC73] text-[#0DBC73] transition`}
              >
                View Profile
              </div>
            </div>
        ...
        <div
          className={`${
            themeMode === "light" ? "border-black/5" : "border-white/5"
          } px-2 py-4 md:p-3 flex items-center gap-2 relative border-t `}
        >
          {/* Image Preview */}
          {imagePreview && (
          ...
          <div className="relative w-full">
            <input
              ...
              className={`${
                themeMode === "light" ? "bg-[#EDEDED]" : "bg-[#222423]"
              } peer w-full px-4 py-2 rounded-full`}
            ...
            {!newMessage && (
              <label
                ...
                className="absolute left-4 top-1/2 transform -translate-y-1/2 
                             ...
                             peer-focus:text-[#0dbc73] peer-focus:text-sm peer-focus:top-0 peer-focus:-translate-y-6 
                             transition-all"
              >
