...
export const EventsView = () => {
  ...
  const getCityBoundary = async (cityName: string) => {
    const url = `https://nominatim.openstreetmap.org/search.php?q=${cityName}&polygon_geojson=1&format=jsonv2`;
  ...
  const handleCardClick = (coordinates: { lat: number; lng: number }) => {
    setViewState({
      longitude: coordinates.lng,
      latitude: coordinates.lat,
      zoom: 15,
    });
    SetFilterEvents(coordinates.lat, coordinates.lng, 15);
  };
      ...
      <div className="w-full h-full grid place-items-center max-h-full relative">
        {loading && (
          <div
            ...
            } absolute inset-0 flex items-center justify-center z-10 rounded-xl`}
          >
          ...
          >
            ...
            {!loading && filteredEvents.length > 0 && (
              <Source
                ...
                cluster={true}
                clusterMaxZoom={14} // Nivel de zoom donde se agrupan
              ...
              >
                <Layer
                  ...
                  type="circle"
                  paint={{
                    "circle-color": "#0DBC73", // Color del círculo de los clusters
                    "circle-radius": [
                      "step",
                      ["get", "point_count"],
                      20,
                      ...
                      750,
                      40,
                    ], // Tamaño del círculo de los clusters basado en el número de puntos
                  }}
                />
                <Layer
                  id="cluster-count"
              ...
              >
                {filteredEvents.map((event) => (
                  <CardEvent
                    key={event.id}
                    event={event}
                    onClickEvent={() => handleCardClick(event.location)}
                  />
                ))}
              </div>
            </div>
