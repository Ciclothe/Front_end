...
export const ProductSection = ({ selectedCategory }: Props) => {
  ...
  const searchText = searchParams.get("search_text")?.toLowerCase() || "";
  ...
  interface Product {
    ...
    category: string;
    picOrientation: string;
    mainPic: string;
  }

  const calculateMatchScore = useCallback(
    (product: Product, searchWords: string[]) => {
      let score = 0;
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (searchText) {
      ...
      baseList = baseList
        .map((product) => {
          const score = calculateMatchScore(product, searchWords);
    ...
    return baseList;
  }, [selectedCategory, searchText, calculateMatchScore]);
